...
fs.readdirSync(path.join(__dirname, "lib", "database")).forEach((plugin) => {
  if (path.extname(plugin).toLowerCase() === ".js") {
    require(path.join(__dirname, "lib", "database", plugin));
  }
});
...
async function Zenox() {
  ...
  conn.ev.on("connection.update", async (s) => {
    ...
    if (connection === "close" && lastDisconnect?.error?.output?.statusCode !== 401) {
      console.log("🔄 Reconnecting due to disconnect...");
      return Zenox(); // Prevent multiple instances
    }
      ...
      try {
        ...
        for (const plugin of plugins) {
          ...
          if (!fs.existsSync(pluginPath)) {
            if (!/^https?:\/\//.test(plugin.dataValues.url)) {
              console.warn(`⚠️ Skipping invalid URL: ${plugin.dataValues.url}`);
              continue;
            }
      ...
      fs.readdirSync("./plugins").forEach((plugin) => {
        if (path.extname(plugin).toLowerCase() === ".js") {
          try {
            ...
            console.error(`❌ Failed to load local plugin ${plugin}:`, err.message);
          }
        }
      });
